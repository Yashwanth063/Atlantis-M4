...
import hdrFilePath from '../../../../assets/glb/bg.hdr';
...
console.log("GlbPlayindDetails",GlbPlayindDetails)
  ...
  useLayoutEffect(() => {
    console.log('deremo count');
    const canvas = document.querySelector('canvas.webgl');
    if (!canvas) {
      console.log('deremo roor');
    ...
    rgbLoader
      .load(hdrFilePath, (texture) => {
        texture.mapping = THREE.EquirectangularReflectionMapping;
        scene.environment = texture;
        // scene.background = texture; // Set HDR texture as background
        ...
        gltfLoader.load(preloadedAssets?.preloadedGLBs?.backgroundGlb?.src, (gltf) => {
          console.log('deremo canavas');
        ...
        // gltfLoader.load(
        //   preloadedAssets.preloadedGLBs.collector,
        //   (gltf) => {
        //     const model = gltf.scene;
        //     model.scale.set(1.2, 1.24, 1.2);
        //     model.rotation.set(0, -6.6, 0);
        //     model.position.set(-1, 0, 6.8);
        ...
        const normalizedPlayerName = (PlayerNameLanguage || "").trim().toLowerCase();
        console.log(normalizedPlayerName,'normalizedPlayerName')
const glbKeys = Object.keys(preloadedAssets?.preloadedGLBs || {});
console.log(glbKeys,'glbKeys')
const normalizedKey1 = glbKeys.find(key => key.toLowerCase() === normalizedPlayerName);

const selectedPC = normalizedKey1 
  ? preloadedAssets.preloadedGLBs[normalizedKey1]?.src 
  : null; // or a default fallback

console.log("Debug:", { PlayerNameLanguage, normalizedKey1, selectedPC });
             console.log("playerselected-profileDatain selectedPC",selectedPC)
             console.log("playerselected-profileDatain normalizedKey1",normalizedKey1)
             console.log("playerselected-profileDatain preloadedAssets",preloadedAssets)
...
(gltf) => {
      const model = gltf.scene;
      model.scale.set(1.2, 1.25, 1.2);
      model.rotation.set(0, 6.5, 0);
      model.position.set(-1, 0, 6.8);
      const playerMixer = new THREE.AnimationMixer(model);
      playerMixerRef.current = playerMixer;
      const animations = gltf.animations;
      ...
      if (idleAnimation) {
        console.log("Playing idle pose immediately to prevent T-pose");
    ...
    (error) => {
      ...
      //   gltfLoader.load(
      //     preloadedAssets.preloadedGLBs.Medieval,
      //     (gltf) => {
      //       const model = gltf.scene;
      //       model.scale.set(1, 1.2, 1);
      //       model.rotation.set(0, 10.9, 0);
      //       model.position.set(0.1, 0.11, 7.2);
      //       const npcMixer = new THREE.AnimationMixer(model);
      //       npcMixerRef.current = npcMixer;
      //       const animations = gltf.animations;
      //       setNpcAnimations(animations);
      //       handleAnimations(npcMixer, animations);
      //       scene.add(model);
      //     },
      //     undefined,
      //     (error) => {
      //       console.error(`Error loading model from :`, error);
      //     }
      //   );
      //   // setModalLoaded(true);
      // });
      ...
      console.log("normalizedKey222:", normalizedKey);
      console.log("selectedNPCcccc",selectedNPC)
...
if (sizes.height > sizes.width) {
  [sizes.width, sizes.height] = [sizes.height, sizes.width];
    ...
    camera.position.set(-1.6, 0.2, 4)
    camera.rotation.y = THREE.MathUtils.degToRad(-20);
    camera.rotation.x = THREE.MathUtils.degToRad(-7);
    scene.add(camera);
 ...
 if (sizes.height > sizes.width) [sizes.width, sizes.height] = [sizes.height, sizes.width];
  ...
  sizes.height = sizes.height;
    ...
    //     playerMixerRef.current.update(clock.getDelta());
    //   }
    //   if (npcMixerRef.current) {
    //     npcMixerRef.current.update(clock.getDelta() * 120);
    //   }
    //   rendererRef.current.render(scene, camera);
    //   if (initialZoomOut && currentScreenId !== 14) {
    //     // Move the camera smoothly from startPosition to endPosition
    //     if (Math.abs(camera.position.z - endPosition.z) > epsilon) {
    //       camera.position.z += zoomSpeed * Math.sign(endPosition.z - camera.position.z);
    //     }

    //     if (Math.abs(camera.position.y - endPosition.y) > epsilon) {
    //       camera.position.y += zoomSpeed * Math.sign(endPosition.y - camera.position.y);
    //     }
    //     if (Math.abs(camera.position.z - endPosition.z) <= epsilon &&
    //       Math.abs(camera.position.y - endPosition.y) <= epsilon) {
  ...
  // useEffect(() => {
  //   if (isZoomComplete && GlbPlayindDetails) {
  //     if (npcMixerRef.current) {
  //       npcMixerRef.current.stopAllAction();
  //       const idleAction = npcMixerRef.current.clipAction(npcAnimations.find((anim) => anim.name === 'IDLE'));
  //       idleAction.reset().play();
  //     }
  //     if (playerMixerRef.current) {
  //       playerMixerRef.current.stopAllAction();
  //       const idleAction = playerMixerRef.current.clipAction(pcAnimations.find((anim) => anim.name === 'IDLE'));
  //       idleAction.reset().play();
  //     }
  //     setTimeout(() => {

  //       if ((GlbPlayindDetails?.whospeak === 'NPC' || GlbPlayindDetails?.whospeak === 'PC') && currentScreenId !== 14) {
  //         handleAnimations(GlbPlayindDetails?.whospeak === 'NPC' ? npcMixerRef.current : playerMixerRef.current, GlbPlayindDetails?.whospeak === 'NPC' ? npcAnimations : pcAnimations);
  //       }
  //     }, 100)
  //   }
  // }, [GlbPlayindDetails]);
  ...
  //   const playAnimation = (mixer, animation, loop = THREE.LoopRepeat, timeScale = 1) => {
  ...
  //   // NPC animations
  //   if (mixer && npcAnimations && GlbPlayindDetails?.whospeak === 'NPC') {
  //     const idleAnimation = npcAnimations.find((anim) => anim.name === 'IDLE');
  //     handleCharacterAnimations(mixer, npcAnimations, idleAnimation);
  //   }

  //   // PC animations
  //   if (mixer && pcAnimations && GlbPlayindDetails?.whospeak === 'PC') {
  //     const idleAnimation = pcAnimations.find((anim) => anim.name === 'IDLE');
  //     handleCharacterAnimations(mixer, pcAnimations, idleAnimation);
  //   }
  // }

  useEffect(() => {
    if (isZoomComplete && currentScreenId === 14) {
      if (npcMixerRef.current) {
        console.log("NPC idle animation found");
  ...
  }, [isZoomComplete, currentScreenId, npcAnimationsRef.current, pcAnimationsRef.current]); 
 ...
 if (npcMixerRef.current && npcAnimationsRef.current) {
  ...
  if (playerMixerRef.current && pcAnimationsRef.current) {
    ...
    playerMixerRef.current.clipAction(idle).reset().play();
      ...
      timeoutId = setTimeout(() => {
        if ((GlbPlayindDetails?.whospeak === 'NPC' || GlbPlayindDetails?.whospeak === 'PC') && currentScreenId !== 14) {
          console.log('bfd',GlbPlayindDetails);
          handleAnimations(
            GlbPlayindDetails?.whospeak === 'NPC' ? npcMixerRef.current : playerMixerRef.current,
            GlbPlayindDetails?.whospeak === 'NPC' ? npcAnimationsRef.current : pcAnimationsRef.current
          );
  ...
  }, [GlbPlayindDetails,isZoomComplete, currentScreenId]);

  function handleAnimations(mixer, animations) {
      //  timeoutIds.current = []; 
      console.log("GlbPlayindDetails111:",GlbPlayindDetails)
    const playAnimation = (mixer, animation, loop = THREE.LoopRepeat, timeScale = 1,fadeDuration=0.5) => {
      console.log('deremo animation');
      const action = mixer.clipAction(animation);
...
BackgroundFeedback.propTypes = {
  ...
  preloadedAssets: PropTypes.shape({
    preloadedGLBs: PropTypes.shape({
      // backgroundGlb: PropTypes.string.isRequired,
      // Medieval: PropTypes.string.isRequired,
...
BackgroundFeedback.defaultProps = {
  modelsActivity: defaultModelsActivity,
}
export default React.memo(BackgroundFeedback);
