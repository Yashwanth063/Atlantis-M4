...
import Magicalgarden1 from 'assets/glb/Magicalgarden.glb';
...
const MagicalGarden = ({
  ...
  useLayoutEffect(() => {
    ...
    const light = new THREE.AmbientLight(0xffffff, 1.71);
    scene.add(light);
    ...
    rgbLoader
      .load(hdrFilePath, (texture) => {
        ...
        gltfLoader.load(preloadedAssets?.preloadedGLBs?.Magicalgarden?.src, (gltf) => {
...
model.position.set(5.5,56.8, -7);
model.rotation.set(0, 2.5, 0);
model.scale.set(2, 2, 2.2);
...
model.position.set(4.8,56.8, -9);
// model.rotation.set(0, 0, 0);
// model.scale.set(3.6, 2.5, 5.5);
  model.scale.set(2, 2.05, 1.5);
...
//old code 
  // camera.position.set(10.9077, 58.5751, -9);
  // kishore new code
  camera.position.set(10.9077, 58.5751, -8.500);
  camera.rotation.set(2.8193106, 1.2701595, -2.8328453);
  // camera.position.set(0, 2, 8);

  
// camera.lookAt(new THREE.Vector3(10.5, 0, 0));  // looks at center between PC and NPC
scene.add(camera);
...
MagicalGarden.defaultProps = {
  modelsActivity: defaultModelsActivity,
}
export default React.memo(MagicalGarden);
