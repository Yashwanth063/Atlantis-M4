...
const defaultModelsActivity = {
  ...
  const RealFactory = ({
    ...
    useLayoutEffect(() => {
      ...
      const light = new THREE.AmbientLight(0xffffff, 1.77);
      scene.add(light);
      ...
      rgbLoader
        .load(hdrFilePath, (texture) => {
          ...
          gltfLoader.load(preloadedAssets?.preloadedGLBs?.Realfactory?.src, (gltf) => {
  ...
  (gltf) => {
     ...
     model.scale.set(1.32, 1.1, 1.1);
    model.rotation.set(0, 2.3, 0);
     model.position.set(-3.2, 10.5, 0.2);
  ...
  if (selectedNPC) {
              ...
              gltfLoader.load(
                ...
                (gltf) => {
                 ...
                 model.scale.set(1.2, 1.1, 1.2);
                model.rotation.set(0, -11.7, 0);
                model.position.set(-3.5, 10.5, -0.9);
...
const camera = new THREE.PerspectiveCamera(
    ...
    // camera.position.set(4.8821, 12.05, -0.0411);
    camera.position.set(3.900, 12.05, 0.0411);

    camera.rotation.set(-1.5707, 1.540, 1.5707);
    scene.add(camera);
// Add camera to scene
scene.add(camera)
...
useEffect(() => {   
 ...
 if (npcMixerRef.current && npcAnimationsRef.current) {
  ...
  RealFactory.defaultProps = {
    modelsActivity: defaultModelsActivity,
  }
  export default React.memo(RealFactory);