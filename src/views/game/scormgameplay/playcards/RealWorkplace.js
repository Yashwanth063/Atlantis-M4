...
import Realworkplace1 from '../../../../assets/glb/Realworkplace.glb';
...
const RealWorkplace = ({
  ...
  useLayoutEffect(() => {
    ...
    const light = new THREE.AmbientLight(0xffffff, 0.09);
    scene.add(light);
...
gltfLoader.load(preloadedAssets?.preloadedGLBs?.Realworkplace?.src, (gltf) => {
...
model.scale.set(1.4, 1.13, 1);
model.rotation.set(0, 2.5, 0);
model.position.set(-0.5, 3.3, -0.1);
...
if (selectedNPC) {
          gltfLoader.load(
            ...
            (gltf) => {
              ...
              model.position.set(-0.4, 3.35, -1.1);
    ...
    const camera = new THREE.PerspectiveCamera(
      ...
      camera.position.set(7.5127, 4.8638, -0.0411);
      camera.rotation.set(-1.5707, 1.55675, 1.57079);
      scene.add(camera);
...
RealWorkplace.defaultProps = {
  modelsActivity: defaultModelsActivity,
}
export default React.memo(RealWorkplace);
