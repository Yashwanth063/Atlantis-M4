...
import { scormupdateLearnerNickName } from 'utils/scormApplication/scormgamePlayService';
...
const Characterspage: React.FC<PlayGamesProps> = ({
  ...
  const selectPlayerClick = async () => {
    ...
    const UpdateLearnerNickname = await scormupdateLearnerNickName(dataString);
  ...
  console.log('LearnerProfile NickName:', learnerPlayList?.LearnerProfile?.nickName);
  
  useEffect(() => {
    console.log('Form state:', formState.name);
  }, [formState.name]);

 console.log("SelectedBackgroundTheme",backgroundtheme)
  ...
  // if ([1, 2, 3].includes(backgroundtheme?.gasId)) {
  // theme = 'Medieval';
  // } else if ([4, 5].includes(backgroundtheme?.gasId)) {
  // theme = 'Future';
  // } else if (backgroundtheme?.gasId === 8) {
  // theme = 'Real';
  // } else if ([6, 7].includes(backgroundtheme?.gasId)) {
  // theme = ['Medieval', 'Future', 'Real'];
  // }
  ...
  console.log("matchingCharacter=preloadedAssets", preloadedAssets);
 ...
 const [selectedIndex, setSelectedIndex] = useState(0); // Initialize index as 0
  ...
  console.log("selected-111-preloadedAssets",preloadedAssets)
  const getTexture = (selected: number): THREE.Texture => {
    console.log("selected-111index",selected)
...
console.log('selectedCharactertttttt',selectedCharacter)
...
console.log("profileINCharacterSelection:",profileData)
...
<img
      ...
      onMouseDown={() => {
        ...
        setSelectedIndex((prevIndex) => {
          const newIndex = 
          // (prevIndex - 1) % preloadedAssets.Player_Image.length;
          // setSelectedCharacter(preloadedAssets.Player_Image[newIndex]); // Update character
          // return newIndex;
...
<img
      ...
      onMouseDown={() => {
        ...
        setSelectedIndex((prevIndex) => {
          // const newIndex = (prevIndex + 1) % preloadedAssets.Player_Image.length;
          // setSelectedCharacter(preloadedAssets.Player_Image[newIndex]); // Update character
          // return newIndex;
