...
import castleGlbb from 'assets/glb/medievalcastle.glb';
...
const Castle = ({
  ...
  useLayoutEffect(() => {
    ...
    gltfLoader.load(preloadedAssets?.preloadedGLBs?.castleGlb?.src, (gltf) => {
    ...
    //     // model.position.set(5.9, 0, 2.3);
    //     model.scale.set(2.2,2.38,2.2);
    //     model.rotation.set(0, 1.8, 0);
    //     model.position.set(5.3, 0, 2.3);
...
(gltf) => {
  const model = gltf.scene;
      //   model.scale.set(2.3,2.43,2.2);
      // model.rotation.set(0, 1.9, 0);
      //   model.position.set(5.3, 0, 2.3);
       model.scale.set(2.4,2.46,2.2);
            model.rotation.set(0,8.5, 0);
            model.position.set(5.3, 0, 2.3);
  const playerMixer = new THREE.AnimationMixer(model);
  playerMixerRef.current = playerMixer;
  const animations = gltf.animations;
  ...
  if (child.isMesh) {
      ...
      if (child.name.toLowerCase().includes("face")) {  
          child.material = new THREE.MeshStandardMaterial({
              ...
              metalness: 0.05,   // Reduce metallic effect
              roughness: 0.25,   // Make skin slightly smoother
              // emissive: new THREE.Color(0x222222),  // Add slight glow
              emissive: new THREE.Color(0xffe0bd),
              emissiveIntensity: 0.5,
          });
          // child.material.color.set(0xffe0bd);
...
(error) => {
  console.log(`Error loading model from :`, error);
    ...
    //     model.position.set(6.05, 0, 0.35);
...
if (selectedNPC) {
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      // model.scale.set(2.2, 2.38, 2.2);
      //     model.rotation.set(0, 0.5, 0);
      //     model.position.set(5.3, 0, 0.35);

          model.scale.set(2.2, 2.48, 2.2);
          model.rotation.set(0, 0.5, 0);
          model.position.set(5.3, 0, 0.35);
...
if (sizes.height > sizes.width) {
  [sizes.width, sizes.height] = [sizes.height, sizes.width];
    ...
    const camera = new THREE.PerspectiveCamera(
      ...
      camera.position.set(12.276, 2.224, 4.065);
      camera.rotation.set(
        41.26 * (Math.PI / 180), 
        71.99 * (Math.PI / 180), 
        -39.84 * (Math.PI / 180) 
