...
useEffect(() => {
    ...
    const AudioDuration =(audio:any,audioUrl:any,gettype?:any,FindCurrentPlayingData?:any)=>
    {
      ...
      audio.addEventListener('loadedmetadata', () => {
        ...
        // Now you can set the duration in your state or other data
    ...
    //               console.log("full url audio normalizedPath:",normalizedPath)

    //             if (normalizedPath !== '') {
    //               const fullUrl = `${API_SERVER}${normalizedPath}`;
    //               console.log("full url audio:",fullUrl)
    ...
    //             if (['Male', 'Others'].includes(profileData?.gender)) {
    //               try {
    //                 const normalizedPath = data?.blockAudioUrl ? data?.blockAudioUrl : '';
    //                 if (normalizedPath !== '') {
    //                   const fullUrl = `${API_SERVER}${normalizedPath}`;
    ...
    //                      console.log('123audioUrl 2=>',audioUrl,',audioBlob=>',audioBlob)
    //                     AudioDuration(audio,audioUrl,type,data);
    //                   }
    //                 }
    //               } catch (error) {
      ...
      if (profileData?.Audiogetlanguage.length !== 0) {
        ...
        if (data && type) {
          if (['Note', 'Dialog', 'Interaction'].includes(type) && AudioOptions?.qpOptionId === '') {
            if (data?.blockRoll !== '999999') {
              try {
          console.log("enterted for audio with data?.blockRoll !== '999999")
                const normalizedPath = data?.blockAudioUrl ? data?.blockAudioUrl : '';
          console.log("enterted normalizedPath",normalizedPath)

                if (normalizedPath !== '') {
                  const fullUrl = `${API_SERVER}${normalizedPath}`;
          console.log("enterted fullUrl",fullUrl)

                  const responseblockText = await fetch(fullUrl);
          console.log("enterted responseblockText",responseblockText)

                  // if (responseblockText.ok)
          console.log("enterted blockText")
          ...
          console.log("enterted setAudioObj",setAudioObj)

                    const audioBlob = await responseblockText.blob();
                    // const audioBlob = "https://192.168.1.35:3000/7b61f48e-c872-43ca-8bf2-62781bbd055f"
          console.log("enterted audioBlob",audioBlob)

                    // Create a URL for the Blob
                    const audioUrl = URL.createObjectURL(audioBlob);
          console.log("enterted audioUrl",audioUrl)

                    // Create an Audio element and set the source to the Blob URL
                    const audio = new Audio(audioUrl);
                    AudioDuration(audio,audioUrl,type,data);
                    console.log("enterted AudioDuration",audio)
                  // }
                }
              } catch (error) {
                console.error('Error fetching data:', error);
                ...
                if (type === 'Interaction') {
                  ...
                  if (optionAudio !== '') {
                    ...
                    if (responseblockText.ok) {
                      ...
                      // Create an Audio element and set the source to the Blob URL
                      const audio = new Audio(audioUrl);
                      console.log('audio.duration int=>',audio.duration)
                      console.log("audio playing in INTERACTION")
                      AudioDuration(audio,audioUrl,type,FindUrl[0]);
                    }
                  }
                } else {
        ...
        >
          <Box display="flex" justifyContent="center">
            ...
            <Box className="story_url_content">
              ...
              <Box className="story_block_btns_box">
                <Box
                    ...
                    <Box position={'relative'}>
                      ...
                      >
                        ...
                        <Box
                          ...
                          className='character_name'
                        >
                          <Text whiteSpace={'nowrap'} overflow={'hidden'} textOverflow={'ellipsis'}>{data.blockRoll === '99999'
                            ? 'Narrator'
                            : data.blockRoll === '999999'
                              ? learnerPlayList?.LearnerProfile?.nickName ?? learnerPlayList?.LearnerProfile?.name 
                              : NonPlayerNameLanguage}</Text>
                        </Box>
                      </Box>
                    </Box>
                    <Box
                      className='dialogue_scroll animateDialog' id='leaderboard_id' bottom={animateDialog ? '-200px' : '16px'} >
                      <Scrollbar>
                        <Box>
