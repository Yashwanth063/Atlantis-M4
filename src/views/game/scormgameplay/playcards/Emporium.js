...
import emporiumGlbb from '../../../../assets/glb/MedievalEmporium.glb';

// import chat from '../../../../../';

/** how we plan to control the GLBs dynamically by updating this props
interface GLBHandlerType {
  preloadedAssets : any;
  modelsActivity:{
    whoIsSpeaking : 'player' | 'npc' | 'narrator'; //on run time who is currently speaking
    playerEmotions: string[]; //['happy','surprised']
    npcEmotions: string[]; //['angry','ugly']
    cameraPosition: {x: 0, y: 0; z: 0}; //
    playerModel : {
      isRequired : true | false; //default false
      postion: {x: 0, y: 0, z: 0} // postion of player model
      rotation : {x: 0, y: 0, z: 0} // rotate the player model
    }
    NpcModel : {
      isRequired : true | false; //default false
      postion: {x: 0, y: 0, z: 0} // postion of player model
  ...
  useLayoutEffect(() => {
    ...
    rgbLoader
      .load(hdrFilePath, (texture) => {
        ...
        gltfLoader.load(preloadedAssets?.preloadedGLBs?.emporiumGlb?.src, (gltf) => {
        ...
        //     // model.rotation.set(0, 0.50, 0);
        //     // model.position.set(-1.5, 3.2, 21);
        //     model.scale.set(1.2,1.21,1.2);
        //     model.rotation.set(0, 0.50, 0);
        //     model.position.set(-1.5, 3.3, 21)
        ...
        console.log("normalizedKey1 :",normalizedKey1)
...
(gltf) => {
      const model = gltf.scene;
          model.scale.set(1.2,1.21,1.2);
          model.rotation.set(0, 7, 0);
          // model.position.set(-1.5, 3.3, 21)
          model.position.set(-0.68, 3.3, 21)
      const playerMixer = new THREE.AnimationMixer(model);
      playerMixerRef.current = playerMixer;
      const animations = gltf.animations;
      // setPcAnimations(animations);
    ...
    (error) => {
      ...
      //     (gltf) => {
      //       const model = gltf.scene;
      //       model.scale.set(1, 1.18, 1);
      //       model.rotation.set(0, -0.8, 0);
      //       model.position.set(-0.5, 3.3, 21);
...
if (selectedNPC) {
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      model.scale.set(1.2,1.21,1.2);
model.rotation.set(0, -0.8, 0);        
// model.position.set(-0.10, 3.3, 21); 
model.position.set(0.55, 3.3, 21); 
...
if (sizes.height > sizes.width) {
  [sizes.width, sizes.height] = [sizes.height, sizes.width];
    ...
    const camera = new THREE.PerspectiveCamera(
      ...
      camera.position.set(-1.438, 5, 25.5);
      camera.rotation.set(
        0.44 * (Math.PI / 180),
        -1.51 * (Math.PI / 180),
        -0.09 * (Math.PI / 180)
