...
import { scormgetGameById, scormgetSkills,scormgetSkillsName } from 'utils/scormGameControl/scormgamecontrol';
...
useEffect(() => {
  ...
  async function fetchSkills() {
    const res = await scormgetSkillsName(gameId);
  ...
  const fetch = async () => {
    const result = await scormgetGameById(formData?.gameId);
    ...
    const res = await scormgetSkills();
    if (res?.status === 'Success') {
      setApSkl(res?.data);
    }
  };
