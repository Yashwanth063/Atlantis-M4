...
import { scormactivityUpdate, scormactivityCreate, scormcreateGamePlayHistory,scormgetTimespent,scormactivitygetlastblock } from "utils/scormApplication/scormgameActivityService";
import { getGameLanguages, getContentRelatedLanguage } from 'utils/scormGameControl/scormgamecontrol';
import { scormupdateLearnerDetails ,updatescormdetails} from 'utils/scormApplication/scormgamePlayService';
...
import pipwerks from '../scormgameplay/scorm_piwark/SCORM_API_wrapper'; 
import { SCORMWrapper }  from "../scormgameplay/scorm_piwark/SCORM_API_wrapper";

import { checkIsMandatoryReplayPromptRequired, isOptionalReplayAllow, getPlayerFinalScore } from './HelperFunctions';
...
const EntirePreview: React.FC<ShowPreviewProps> = ({
 ...
 timer,
  ...
  const [status,Setstatus] = useState<any>('failed');
  const [completionstatus,Setcompletionstatus] = useState<any>('not attempted');
  const EnumType = {
    BGM: 'bgm',
    VOICE: 'voice',
  };
  ...
  const [learnerPlayList, setLearnerPlayingDetails] = useState<any>({
    playerId: scormDetails.scormLearnerId,
    playerType: 'scormlearner',
    GameId: parseInt(learner_game_play_id),
    screenIdSeq: [],
    LearnerProfile: {
      name: gameInfo?.learnerDetails?.scormlenUserName ?? '',
      age: gameInfo?.learnerDetails?.scormlenAge ?? '',
      companyId: gameInfo?.learnerDetails?.scormlenCompanyId ?? '',
      CountryId: gameInfo?.learnerDetails?.scormlenCountryId ?? '',
      gender: gameInfo?.learnerDetails?.scormlenGender ?? '',
      Department: gameInfo?.learnerDetails?.scormlenDepartment ?? '',
      language: 1,
      education: gameInfo?.learnerDetails?.scormlenEducation ?? '',
      mailId: gameInfo?.learnerDetails?.scormlenMail ?? '',
      score: '',
      nickName: gameInfo?.learnerDetails?.scormlenNickName ?? '',
      region: gameInfo?.learnerDetails?.scormlenRegion ?? '',
      allTimeScore: '',
    },
   CurrentBlock:'',
  ...
  console.log(learnerPlayList,'learnerPlayListinentirepreview')
...
const [NonPlayerNameLanguageId, setNonPlayerNameLanguageId] = useState(gameInfo?.gameData?.gameNonPlayingCharacterId);
  console.log("NonPlayerNameLanguageId",NonPlayerNameLanguageId)
...
useEffect(() => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'hidden' || document.hasFocus()) {
        ...
        if (currentScreenId && currentScreenId !== 10) {
          ...
          if (backgroundBgmRef?.current && backgroundBgmRef?.current?.paused && ![2, 10, 6, 4].includes(currentScreenId) && backgroundBgmRef.current?.src !== '' && backgroundBgmRef.current?.src !== null && backgroundBgmRef.current?.src !== undefined) {
            try {
  ...
  const UpdateLearnerData = async () => {
    ...
    const UpdateLearnerDetails = await scormupdateLearnerDetails(dataString);
    setAudioObj((prev) => ({
      ...prev,
      Voicevolume: learnerPlayList?.audioVolumeValue?.voiceVolume ?? 0.5,
      BGMvolume: learnerPlayList?.audioVolumeValue?.bgmVolume ?? 0.5,
  ...
  const calculateScores = (data:any) => {
    // Initialize totals for min, max, and raw scores
    console.log('data get=>',data)
    let totalMin = 0;
    let totalMax = 0;
    let totalRaw = 0;
  
    // Loop through the data and accumulate scores
    data.forEach((item:any) => {
      console.log('item.minScore=>',typeof(item.minScore));
      totalMin += item.minScore;
      totalMax += item.TotalScore;
      totalRaw += item.earningScore;
    });
...
const UpdateScormData = async()=>
  {
    const GetCompoQuest = profile.completedLevels;
    let findCompletQuestData:any = [];
    const questScores: any = {};
    let scores = questState[profile?.currentQuest] == 'Started'
    ? profile?.score
    ...
    gameInfo?.gameQuest.map((element:any) => {
      if(GetCompoQuest.includes(String(element.gameQuestNo)))
      {
        const sendData = {
          questno:element.gameQuestNo,
          TotalScore:element.gameTotalScore,
          minScore:element.gameMinScore ?? 0 ,
          earningScore: questScores[parseInt(element.gameQuestNo)] ?? 0,
        }
        findCompletQuestData.push(sendData);
      }
    });
    if(findCompletQuestData)
    {
      const getSCore = calculateScores(findCompletQuestData);
      console.log('getSCore=>',getSCore)
      
      const data ={
        registrationId :scormDetails?.registrationId,
        learnerMail :scormDetails?.learner?.id,     
        max :getSCore.max,
        min :getSCore.min,
        raw :getSCore.raw,
        success: status,
     ...
     const  max =getSCore.max;
     const  min =getSCore.min;
     const  raw =getSCore.raw;
      const completionStatus = completionstatus; // e.g., "completed" or "incomplete"
    const successStatus = status; // e.g., "passed" or "failed"
    console.log('getSCore=>max',max)
    console.log('getSCore=>raw',raw)
    console.log('getSCore=>',getSCore)
    // Update SCORM Cloud for this learner

    // pipwerks.SCORM.setScore(getSCore.raw, getSCore.max);
    SCORMWrapper.sendScormScore(raw, max);
    // if (getSCore.raw >= getSCore.min) { // Example: Pass if score is 80% or higher
    //     pipwerks.SCORM.setCompletionStatus("passed");
    // } else {
    //     pipwerks.SCORM.setCompletionStatus("failed");
    // }
//     pipwerks.SCORM.setScoring(raw,max);
//     // pipwerks.SCORM.finish(); // End SCORM session
//     console.log("SCORM Raw Score:", pipwerks.SCORM.getValue("cmi.core.score.raw"));
// console.log("SCORM Completion Status:", pipwerks.SCORM.getValue("cmi.core.lesson_status"));
// console.log("SCORM Success Status:", pipwerks.SCORM.getValue("cmi.core.success_status"));

      const dataString = JSON.stringify(data);
      console.log("dataString data:",dataString)
      const UpdateScormrDetails = await updatescormdetails(dataString);
    }
  }

      console.log("GlbPlayindDetails",GlbPlayindDetails)
  ...
  useEffect(() => {
  console.log("Updated GlbPlayindDetails:", GlbPlayindDetails);
}, [GlbPlayindDetails]);
  useEffect(() => {
    UpdateScormData();
  }, [completionstatus,status,profile?.replayScore,profile?.replayScore]);
console.log("scormDetails",scormDetails)
// const learnerMail
console.log("scormDetailsscormDetails?.learner?.id",scormDetails?.learner?.id)
  useEffect(() => {
  ...
  useEffect(() => {
    if (!gameInfo?.gameData?.gameIntroMusicName && !gameInfo?.gameData?.gameIntroMusic) {
      ...
      if (
        ...
        ![2, 3, 4, 5, 6, 7, 10, 0,  14].includes(currentScreenId)
      ) {
        setAudioObj((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    ...
    if (audio !== '') {
      if (![2, 3, 4, 5, 6, 7, 10, 0, 14].includes(currentScreenId)) {
        setAudioObj((prev) => ({
          ...prev,
          url: audio,
          type: EnumType.BGM,
...
useEffect(() => {
    ...
    console.log("setAudioObj",audioObj)
  console.log("setVoiceIds",voiceIds)
  useEffect(() => {
    if (![2, 3, 4, 5, 6, 7, 10, 0, 14].includes(currentScreenId)) {
      if (!gameInfo?.gameData?.gameIntroMusicName && !gameInfo?.gameData?.gameIntroMusic) {
        setAudio('');
...
const LastModiPrevData = async(current: any = null) => {
    ...
    if (type !== 'Interaction') {
      ...
      const result = await scormactivityUpdate(datas, actId1);
  ...
  useEffect(()=>{
    const fetchgetTimespent = async ()=>{
      try {
        let datatoSend={
          gameid:learner_game_play_id,
          learnerId:scormDetails.scormLearnerId,
        }
         const response = await scormgetTimespent(JSON.stringify(datatoSend));
 ...
 const foundOnlyOption = getscoreseq?.choosedoption;
      ...
      if (result.status !== 'Success') {
        ...
        const histdata = {
          ...
          histOption: selectedOption && selectedOption[0]?.choosedoption ? selectedOption[0]?.choosedoption : null,
          histScore: getScore !== null && !isNaN(getScore) ? getScore : null, 
          learnerid:scormDetails.scormLearnerId,
        };
        const stringhistdata = JSON.stringify(histdata)
        if (learner_game_play_id) {
          const historyResult = await scormcreateGamePlayHistory(stringhistdata);
        }
      }
...
console.log("setAudioObj-scorm",audioObj)
  useEffect(() => {
    if (voiceRef.current) {
      voiceRef.current.pause();
    }
    ...
    if (
      ...
      if (backgroundBgmRef.current && ![2, 3, 4, 5, 6, 7, 10, 0, 14].includes(currentScreenId) &&
      backGroundBgmscreens.includes(currentScreenId)) {
        try {
          backgroundBgmRef.current.play().catch((error) => {
          }); 
  ...
  const handleReplayButtonClick = async(replayType: string) => {
    const updatedFeedbackList = feedbackList.filter(feedback => feedback.quest != profile.currentQuest);
    ...
    //   }
      ...
      setProfile((prev: any) => ({
        ...
        console.log("result from activity",result)
    }
...
const lastBlockresult = await scormactivitygetlastblock(datas1);
      ...
      {isAuthFailed.status === false ?
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: motionEffect ? 0 : 1 }}>
          ...
          >
            ...
            <>
             {/* {(currentScreenId === 2 || currentScreenId === 9 || currentScreenId === 14) && (
                <BackgroundGLB
                  preloadedAssets={preloadedAssets}
