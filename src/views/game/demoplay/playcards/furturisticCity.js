...
import futuristicGlbb from '../../../../assets/glb/Futuristiccity.glb';
  ...
  useLayoutEffect(() => {
    ...
    rgbLoader
      .load(hdrFilePath, (texture) => {
        ...
        gltfLoader.load(preloadedAssets?.preloadedGLBs?.furturisticCity?.src, (gltf) => {
...
(gltf) => {
    ...
    model.rotation.set(0, 2.10, 0);
//     // model.position.set(28.8, 52.4, 2.4);
    model.position.set(28.8, 52.47, 2.4);
  ...
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      model.rotation.set(0, 0.7, 0);
       model.position.set(28.7, 52.5, 1.2);
...
if (sizes.height > sizes.width) {
  [sizes.width, sizes.height] = [sizes.height, sizes.width];
    ...
    const camera = new THREE.PerspectiveCamera(
      ...
      camera.position.set(32.723, 53.829, 2.136);
      camera.rotation.set(1.682, 1.507, -1.682);
    scene.add(camera);
...
FuturisticCity.defaultProps = {
  modelsActivity: defaultModelsActivity,
}
export default React.memo(FuturisticCity);
