...
import futuristicGlbb from '../../../../assets/glb/Futuristiclab.glb';
...
const FuturisticLab = ({
  ...
  gltfLoader.load(preloadedAssets?.preloadedGLBs?.futuristicGlb?.src, (gltf) => {
...
model.position.set(0.51, 0.35, 0);
...
if (selectedNPC) {
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      model.scale.set(1, 1.02, 1);
       model.rotation.set(0, 0.8, 0);
      model.position.set(0.37, 0.35, -1);
  ...
  console.error(`Invalid NPC selection: ${NonPlayerNameLanguage}`);
   ...
   sizes = {
    ...
    camera.position.set(4.445, 1.365, -0.041);
    camera.rotation.set(
      90.0 * (Math.PI / 180), // Convert 18.41 degrees to radians
      87.41 * (Math.PI / 180), // Convert 83.7 degrees to radians
      -90.0 * (Math.PI / 180) // Convert -18.3 degrees to radians
    );
    scene.add(camera);
...
FuturisticLab.defaultProps = {
  modelsActivity: defaultModelsActivity,
}
export default React.memo(FuturisticLab);
