...
const camera_start_position = new THREE.PerspectiveCamera(
  75,
...
const Office = ({
  ...
  NonPlayerNameLanguageId, // Default value for modelsActivity
}) => {
  ...
  useLayoutEffect(() => {
    ...
    rgbLoader.load(hdrFilePath, (texture) => {
      ...
      gltfLoader.load(
        preloadedAssets?.preloadedGLBs?.OfficeGlb?.src,
        (gltf) => {
 ...
 console.log("playerselected-selectedPC",selectedPC)
 console.log("playerselected-normalizedKey1",normalizedKey1)
 console.log("playerselected-preloadedAssets",preloadedAssets)
   if(selectedPC){
    gltfLoader.load(
selectedPC,
(gltf) => {
  ...
  model.scale.set(2, 2.18, 2);
  model.rotation.set(0, 8.3, 0);
  model.position.set(-16.8, 0.18, -13.06);
  ...
  if (child.isMesh) {
  child.material.envMapIntensity = 9.5; 
  }
});
...
model.scale.set(2, 2.25, 2);
model.rotation.set(0, 0.50, 0);
model.position.set(-16.45, 0.25, -15.3);
      ...
      model.traverse((child) => {
  if (child.isMesh) {
    child.material.envMapIntensity = 10; // Reduce environment map effect
  }
});
            scene.add(model);
          },
    ...
    camera.position.set(-9.800, 2.795, -11.821);
    camera.rotation.set(
      18.41 * (Math.PI / 180), // Convert 18.41 degrees to radians
      83.7 * (Math.PI / 180), // Convert 83.7 degrees to radians
      -18.3 * (Math.PI / 180), // Convert -18.3 degrees to radians
    );
    scene.add(camera);
 ...
 if (sizes.height > sizes.width) [sizes.width, sizes.height] = [sizes.height, sizes.width];
  ...
  useEffect(() => {
    if (currentScreenId === 14) {
      if (npcMixerRef.current) {
        ...
        const idleAnimation = npcAnimationsRef.current.find(
          (anim) => anim.name === 'IDLE POSE',
        );
        if (idleAnimation) {
      ...
      if (playerMixerRef.current) {
        ...
        const idleAnimation = pcAnimationsRef.current.find(
          (anim) => anim.name === 'IDLE POSE',
        );
        if (idleAnimation) {
  ...
  //     if (npcMixerRef.current) {
  //       const idleAction = npcMixerRef.current.clipAction(
  //         npcAnimationsRef.current.find((anim) => anim.name === 'IDLE POSE'),
  //       );
  //       idleAction.reset().play();
  //     }
  //     if (playerMixerRef.current) {
  //       const idleAction = playerMixerRef.current.clipAction(
  //         pcAnimationsRef.current.find((anim) => anim.name === 'IDLE POSE'),
  //       );
  //       idleAction.reset().play();
  //     }
  ...
  //       if (
  //         (GlbPlayindDetails?.whospeak === 'NPC' ||
  //           GlbPlayindDetails?.whospeak === 'PC') &&
  //         currentScreenId !== 14
  //       ) {
  ...
  //           GlbPlayindDetails?.whospeak === 'NPC'
  //             ? npcMixerRef.current
  //             : playerMixerRef.current,
  //           GlbPlayindDetails?.whospeak === 'NPC'
  //             ? npcAnimationsRef.current
  //             : pcAnimationsRef.current,
  //         );
  //       }
  //     }, 300);
  //   }
...
useEffect(() => {   
 ...
 if (npcMixerRef.current && npcAnimationsRef.current) {
  ...
  function handleAnimations(mixer, animations) {
    ...
    const handleCharacterAnimations = (
      mixer,
      charAnimations,
      idleAnimation,
    ) => {
      ...
      if (multiAction.length > 1) {
        const anim1 =
          charAnimations.find((anim) => anim.name === multiAction[0]) ||
          idleAnimation;
        const anim2 =
          charAnimations.find((anim) => anim.name === multiAction[1]) ||
          idleAnimation;
        playSequence(mixer, [anim1, anim2], idleAnimation, duration);
      } else {
        const targetAnimation =
          charAnimations.find((anim) => anim.name === emotion) || idleAnimation;
    ...
    if (
      mixer &&
      (!GlbPlayindDetails?.whospeak ||
        GlbPlayindDetails?.whospeak === 'Narrator' ||
        currentScreenId === 14)
    ) {
